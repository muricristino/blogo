defmodule BlogoWeb.Blocks do
  ...
  def render_blocks(assigns) do
    ...
    <div :for={block <- @blocks} class="row">
      <.block block={block} />
      <aside class="mnote" :if={block["note"]}><%= inline(block["note"]) %></aside>
    ...
    <figure>
      <div class="card tbl-wrap" style="overflow:hidden">
        <table class="tbl">
          <thead>
            <tr>
              <th :for={{h, i} <- Enum.with_index(@block["headers"] || [])}
                  scope="col" style={i in @numeric && "text-align:right"}><%= h %></th>
            </tr>
          </thead>
          <tbody>
            <tr :for={row <- @block["rows"] || []}>
              <td :for={{cell, i} <- Enum.with_index(row)} class={i in @numeric && "r"}><%= inline(cell) %></td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  def block(%{block: %{"type" => "quote"}} = assigns) do
    ~H"""
    <blockquote class="pull" style="border-left:3px solid color-mix(in srgb,var(--accent) 40%,transparent);padding-left:20px">
      <%= inline(@block["text"]) %>
      <cite :if={@block["cite"]} class="small" style="display:block;margin-top:10px;font-style:normal;font-family:'IBM Plex Sans',sans-serif">
        <%= @block["cite"] %>
      </cite>
    </blockquote>
    """
  ...
  def block(%{block: %{"type" => "keynumbers"}} = assigns) do
    ~H"""
    <div class="card" style="padding:22px 24px;display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:20px">
  ...
  defp numeric?(v) when is_binary(v), do: Regex.match?(~r/^[\s]*[<>~]?[\s]*(US\$|R\$)?[\s]*[\d.,]+(x|%|\s*(ms|s|min|tokens?))?[\s]*$/u, v)
